...
export class CBEsRoleService{
  private readonly baseURL = 'http://192.168.1.11:8000/api/CBEsRole'
  ...
  EditRoleGroup(RoleGroup:CBEsRole): Observable<any> {
    const token = localStorage.getItem('Token'); // Replace 'token' with your actual token key in LocalStorage
    const headers = new HttpHeaders()
      .set('Content-Type', 'application/json')
      .set('Authorization', `Bearer ${token}`); // Use Bearer token
    return this.httpClient.put<Response>(`${this.baseURL}/RoleWithUsers`, RoleGroup, { headers });
  }
